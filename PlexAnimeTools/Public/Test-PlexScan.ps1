...
function Test-PlexScan {
    ...
    process {
        ...
        # Use comprehensive structure detection
        $structure = Get-FolderStructureType -FolderPath $Path
        
        if ($structure.SeasonFolders.Count -gt 0) {
            $score += 3
            if ($Detailed) {
                Write-LogMessage "  [OK] Found $($structure.SeasonFolders.Count) season folder(s) - Pattern: $($structure.Pattern)" -Level Success
            }
            
            # Warn if non-standard structure
            if ($structure.Type -ne 'StandardPlex') {
                $warnings += "Non-standard season folder structure detected: $($structure.Pattern)"
                if ($Detailed) {
                    Write-LogMessage "  [WARNING] Non-standard structure: $($structure.Type)" -Level Warning
                }
            }
            
            $seasonFolders = $structure.SeasonFolders
        }
        elseif ($structure.Type -eq 'FlatSeries') {
            # Flat structure - all episodes in root
            $score += 1  # Give partial credit
            $warnings += "No season folders - all episodes in root folder"
            if ($Detailed) {
                Write-LogMessage "  [WARNING] Flat structure detected - episodes should be in season folders" -Level Warning
            }
            $seasonFolders = @()
        }
        elseif ($structure.Type -eq 'SingleMovie') {
            # Single movie file is OK
            $score += 3
            if ($Detailed) {
            ...
            $issues += "No season folders or valid structure found"
            if ($Detailed) {
                Write-LogMessage "  [ERROR] Invalid structure: $($structure.Type)" -Level Error
            }
            $seasonFolders = @()
        }
        ...
        # Check files in root for flat structures or movies
        if ($structure.Type -eq 'FlatSeries' -or $structure.Type -eq 'SingleMovie') {
            $rootFiles = $structure.RootVideos
            $fileCount = $rootFiles.Count
            
            if ($structure.Type -eq 'SingleMovie') {
                # Movie - check for year or (Movie) in name
                if ($rootFiles[0].Name -match '\(\d{4}\)' -or $rootFiles[0].Name -match '\(Movie\)') {
                    $validFiles = 1
                }
                else {
                    $warnings += "Movie file should include year or (Movie) in name"
                }
            }
            else {
                # Flat series - episodes should still have proper naming
                foreach ($file in $rootFiles) {
                    if ($file.Name -match 'S\d{2}E\d{2}' -or $file.Name -match 'Episode\s+\d+' -or $file.Name -match '-\s*\d+\s*-') {
                        $validFiles++
                    }
                    else {
                        $warnings += "Flat structure file should have episode numbers: $($file.Name)"
                    }
                }
            }
        }
