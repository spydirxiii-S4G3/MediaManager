...
function Get-EpisodeNumber {
    ...
    try {
        # Try various patterns (ordered from most specific to most generic)
        $patterns = @(
            'Episode\s+(\d{1,4})',               # Episode 05, Episode 123 (with space)
            '(?:S\d+E|E|EP)(\d{1,4})',          # S01E05, E05, EP05 (no space)
            '\s-\s(\d{1,4})\s',                  # - 05 -
            '\[(\d{1,4})\]',                     # [05]
            '_(\d{1,4})_',                       # _05_
            '^(\d{1,4})\s',                      # starts with number
            '\s(\d{1,4})\.(?:mkv|mp4|avi)',     # ends with number before extension
            '\s(\d{1,4})\s',                     # space 05 space
            '(?:^|\D)(\d{1,4})(?:\D|$)'         # any number surrounded by non-digits (fallback)
        )
